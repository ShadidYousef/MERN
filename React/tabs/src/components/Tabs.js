...
const Tabs = (props) => {
  ...
  return (
    <div style={{width:"90%", height:"100px", margin:"0 auto", display:"flex"}}>
        {props.array.map((tab, i) => <div key={i} style={ divStyle } onClick={e => {props.assignTab(tab); e.target.style.backgroundColor="Red";e.target.style.color="white";}}>{tab.label}</div>)}
    </div>
  )
}
